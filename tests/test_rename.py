#!/usr/bin/env python3
"""Kiểm `rename.py` — mỗi chốt phải bắt đúng ca hỏng nó sinh ra để bắt.
...
SCRIPT = os.path.join(REPO, "skills", "code-optimize", "scripts", "rename.py")
...
def chay(goc, *args):
    r = subprocess.run([sys.executable, SCRIPT, "--root", goc] + list(args),
...
def chay_trong(tmp):
    ...
    goc = dung_repo(tmp)
    truoc = chup(goc)
    ma, ra = chay(goc, "--old", "so", "--new", "dem_so")
    kiem("exit 0, liet ke cho se dung", ma == 0 and "a/x.py" in ra and "a/y.php" in ra, ra[-300:])
    kiem("khong doi gi", chup(goc) == truoc)

    print("\n[2] RANH GIOI TU — doi `so` KHONG duoc dung `file_count`, `params`")
    ma, ra = chay(goc, "--old", "so", "--new", "dem_so", "--write")
    ...
    kiem("`file_count` va `params` GIU NGUYEN", '"file_count"' in x and '"params"' in x, x)
    ...
    git(goc, "-c", "user.name=t", "-c", "user.email=t@t", "commit", "-qam", "switched")

    print("\n[3] VA CHAM — ten moi da ton tai thi TU CHOI")
    ma, ra = chay(goc, "--old", "dem_so", "--new", "counts", "--write")
    kiem("exit 3 NAME_COLLISION", ma == 3 and "NAME_COLLISION" in ra, ra[-300:])
    ...
    ma, ra = chay(goc, "--old", "dem_so", "--new", "so_moi", "--write", "--quoted-only")
    ...
    truoc = chup(goc)
    ma, ra = chay(goc, "--old", "so_moi", "--new", "hong_roi", "--write", "--check",
                  sys.executable + " -c \"import sys; sys.exit(1)\"")
    kiem("exit 6 CHECK_FAILED", ma == 6 and "CHECK_FAILED" in ra, ra[-400:])
    kiem("cay ve dung tung byte", chup(goc) == truoc,
         str([k for k in truoc if truoc[k] != chup(goc).get(k)]))
    ma, ra = chay(goc, "--old", "so_moi", "--new", "ok_roi", "--write", "--check",
    ...
    git(goc, "add", "a/z.txt")
    ma, ra = chay(goc, "--old", "ok_roi", "--new", "bat_ky", "--write")
    ...
    ma, ra = chay(goc, "--old", "khong_ton_tai_dau", "--new", "x")
