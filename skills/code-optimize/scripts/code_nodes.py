...
`graph_gate.py` đối chứng giả thuyết đó với ADN chụp từ một WordPress đang chạy.
...
Mọi thứ không phân giải được thì ĐẾM VÀO `unresolved`, không đoán. Con số
...
mọi kết luận kiểu "không ai gọi" phải đọc kèm con số ấy.
    ...
    python code_nodes.py --theme "duong/dan/theme" [--out graph.json]
"""
import argparse
import json
import os
...
def quet_file(goc, duoi):
    ...
    for dp, _dn, fn in os.walk(goc):
        if any(x in dp.replace(os.sep, "/") for x in SKIP_DIRS):
            continue
        for f in sorted(fn):
            if f.endswith(duoi):
...
RE_FUNC = re.compile(r"^[ \t]*function[ \t]+([A-Za-z_][A-Za-z0-9_]*)[ \t]*\(", re.M)
...
#     mà là `unresolved` phồng lên 2 vì lỗi của chính tool — tức con số đo độ
...
#     tham số và mảng dependency không bao giờ được đọc — cạnh `depends_on` im lặng
#     biến mất. Lần này tool không báo mù ở đâu cả: nó báo `unresolved = 0`
...
RE_ENQUEUE_OPEN = re.compile(r"\bwp_(?:enqueue|register)_(script|style)\s*\(")
RE_HOOK_OPEN = re.compile(r"\b(add_action|add_filter|add_shortcode)\s*\(")
...
def tach_tham_so(s, i_mo):
    ...
    if i_mo >= len(s) or s[i_mo] != "(":
        return None, None
    sau, muc, params, hien_tai = i_mo + 1, 1, [], []
    nhay = None
    i = sau
    while i < len(s):
        c = s[i]
        ...
        if c in "'\"":
            ...
            muc -= 1
            if muc == 0:
                params.append("".join(hien_tai).strip())
                return [t for t in params], i + 1
            hien_tai.append(c)
        elif c == "," and muc == 1:
            params.append("".join(hien_tai).strip())
            hien_tai = []
...
def dung(goc_theme):
    ...
    for f, s in php.items():
        for h in RE_FUNC.findall(bo_script(s)):
    ...
    unresolved = {
        ...
        "dynamic_callback": [],
        "dynamic_handle": [],
        "dynamic_template_part": [],
    }

    def node(nid, loai, **thuoc_tinh):
        if nid not in nodes:
            nodes[nid] = {"id": nid, "kind": loai, **thuoc_tinh}
        return nid

    def canh(tu, den, loai, f, dong, certain=True, note=None):
        edges.append({"from": tu, "to": den, "kind": loai,
                      "source": f, "line": dong, "certain": certain,
                      **({"note": note} if note else {})})

    def line_count(s, vi_tri):
        return s.count("\n", 0, vi_tri) + 1

    for f, s in php.items():
        node("file:" + f, "file", path=f, line_count=len(s.splitlines()))

    for ten, f in khai_ham.items():
        node("func:" + ten, "func", declared_in=f)
        canh("file:" + f, "func:" + ten, "declares", f, None)

    for f, s in php.items():
        sach = bo_ghi_chu_giu_chuoi(s)

        # ── require / include
        for m in RE_REQUIRE.finditer(sach):
            dich = giai_require(m.group(1), ten_file)
            if dich:
                for d in dich:
                    canh("file:" + f, "file:" + d, "require", f, line_count(sach, m.start()),
                         certain=len(dich) == 1,
                         note=None if len(dich) == 1 else "mo ho: khop nhieu file")
            else:
                unresolved["dynamic_require"].append(
                    {"file": f, "line": line_count(sach, m.start()),
                     "expr": m.group(1).strip()[:90]})
        ...
        for ten, g in khai_ham.items():
            ...
            if re.search(r"(?<![A-Za-z0-9_$>])" + re.escape(ten) + r"\s*\(", khong_chuoi):
                canh("file:" + f, "func:" + ten, "calls", f, None)

        # ── đăng ký hook: ĐÂY là cạnh mà không tool tĩnh nào giải được trọn vẹn
        for m in RE_HOOK_OPEN.finditer(sach):
            ham_dk = m.group(1)
            params, _ = tach_tham_so(sach, m.end() - 1)
            dong = line_count(sach, m.start())
            if params is None or len(params) < 2:
                unresolved["dynamic_callback"].append(
                    {"file": f, "line": dong, "func": ham_dk,
                     "expr": "UNPARSED_ARGS"})
                continue
            bieu_hook = params[0]
            bieu_cb = params[1]
            bieu_ut = params[2] if len(params) > 2 else None
            ten_hook = chuoi_thuan(bieu_hook)
            ten_cb = chuoi_thuan(bieu_cb)
            priority = so_nguyen_thuan(bieu_ut) if bieu_ut else (10 if ham_dk != "add_shortcode" else None)

            if ten_hook is None:
                unresolved["dynamic_hook"].append(
                    {"file": f, "line": dong, "expr": bieu_hook.strip()[:90],
                     "func": ham_dk})
            if ten_cb is None:
                unresolved["dynamic_callback"].append(
                    {"file": f, "line": dong, "expr": bieu_cb.strip()[:90],
                     "func": ham_dk})
            if ten_hook is None or ten_cb is None:
                continue

            node("hook:" + ten_hook, "hook", ten=ten_hook)
            node("func:" + ten_cb, "func", declared_in=khai_ham.get(ten_cb))
            canh("hook:" + ten_hook, "func:" + ten_cb, "registration", f, dong,
                 note=f"{ham_dk} p{priority}" if priority is not None else ham_dk)
            canh("file:" + f, "hook:" + ten_hook, "registers", f, dong)

        # ── phát hook
        for m in RE_EMIT.finditer(sach):
            ten_hook = chuoi_thuan(m.group(2))
            dong = line_count(sach, m.start())
            if ten_hook is None:
                unresolved["dynamic_hook"].append(
                    {"file": f, "line": dong, "expr": m.group(2).strip()[:90],
                     "func": m.group(1)})
                continue
            node("hook:" + ten_hook, "hook", ten=ten_hook)
            canh("file:" + f, "hook:" + ten_hook, "emits", f, dong, note=m.group(1))

        # ── asset handle + dependency
        for m in RE_ENQUEUE_OPEN.finditer(sach):
            loai_asset = m.group(1)
            params, _ = tach_tham_so(sach, m.end() - 1)
            dong = line_count(sach, m.start())
            if not params:
                unresolved["dynamic_handle"].append(
                    {"file": f, "line": dong, "expr": "UNPARSED_ARGS"})
                continue
            bieu_handle = params[0]
            bieu_deps = params[2] if len(params) > 2 else None
            handle = chuoi_thuan(bieu_handle)
            if handle is None:
                unresolved["dynamic_handle"].append(
                    {"file": f, "line": dong, "expr": (bieu_handle or "").strip()[:90]})
                continue
            node("asset:" + handle, "assets", ten=handle, kieu=loai_asset)
            canh("file:" + f, "asset:" + handle, "enqueue", f, dong, note=loai_asset)
            for d in re.findall(r"'([^']*)'|\"([^\"]*)\"", bieu_deps or ""):
                ten_dep = d[0] or d[1]
                if ten_dep:
                    node("asset:" + ten_dep, "assets", ten=ten_dep, kieu=loai_asset)
                    canh("asset:" + handle, "asset:" + ten_dep, "depends_on", f, dong)
        ...
        for m in RE_TEMPLATE_PART.finditer(sach):
            ...
            if goc_part is None:
                unresolved["dynamic_template_part"].append(
                    {"file": f, "line": dong, "expr": m.group(1).strip()[:90]})
                continue
            ...
            if tim_thay:
                ...
                unresolved["dynamic_template_part"].append(
                    {"file": f, "line": dong,
                     "expr": goc_part + (("-" + hau) if hau else ""),
                     "note": "khong tim thay file dich"})

    tong_chua_giai = sum(len(v) for v in unresolved.values())
    return {
        "version": 1,
        "theme": os.path.basename(goc_theme.rstrip("/\\")),
        "nodes": [nodes[k] for k in sorted(nodes)],
        "edges": sorted(edges, key=lambda e: (e["kind"], e["from"], e["to"], e["line"] or 0)),
        "unresolved": unresolved,
        "unresolved_count": tong_chua_giai,
    }
...
def in_ra(g):
    theo_loai = {}
    for n in g["nodes"]:
        theo_loai[n["kind"]] = theo_loai.get(n["kind"], 0) + 1
    canh_loai = {}
    for e in g["edges"]:
        canh_loai[e["kind"]] = canh_loai.get(e["kind"], 0) + 1
    ...
    if t == 0:
        ...
        print("  Chốt duy nhất nói được đồ thị có đúng hay không là graph_gate.py, đối chứng")
        print("  với ADN chụp từ WordPress đang chạy.")
    else:
        print(f"  !! unresolved = {t} — mỗi mục là một CẠNH BỊ THIẾU")
        for k, v in g["unresolved"].items():
            if not v:
                continue
            print(f"     {k}: {len(v)}")
            for m in v[:6]:
                print(f"        {m['file']}:{m.get('line')}  {m.get('expr')}")
            if len(v) > 6:
...
def main():
    ...
    ap.add_argument("--out", default="", help="ghi graph ra file JSON")
    a = ap.parse_args()

    goc = a.theme.rstrip("/\\")
    if not os.path.isdir(goc):
        print("NOT_CHECKABLE: khong thay thu muc " + goc)
        return 4
